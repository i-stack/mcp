...
**MCP 配置同步**：用**一份** `mcp-servers.json`（本仓库中的 MCP 服务清单）作为数据源，**同时**同步到 **Cursor**、**Codex** 与 **Claude Code**，避免在多个工具里重复维护 MCP 列表。
...
| `sync_mcp.py` | 从 `mcp-servers.json` 生成 Codex 用 TOML，并合并进 `config.toml` |
...
单独执行 `sync_mcp.py` 时仍会更新 Codex 侧配置；Cursor 需自行保证 `~/.cursor/mcp.json` 指向本仓库的 `mcp-servers.json`（或直接运行 `sync_all.sh`）。
